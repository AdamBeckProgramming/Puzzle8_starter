...
public class PuzzleBoardView extends View {
    ...
    public void solve() {
        PriorityQueue<PuzzleBoard> pq = new PriorityQueue<PuzzleBoard>(1000, new PuzzleComparator(){});
        puzzleBoard.reset();
        pq.add(puzzleBoard);
        while(!pq.isEmpty()){
            PuzzleBoard tempBoard = pq.poll();
            int prio = tempBoard.priority() - tempBoard.getSteps();
            Log.d("before if", ""+prio);
            if(prio == 0) {
                Log.d("Solve", "Was solved!" + tempBoard.resolved());
                // = tempboard.getPreviousBoard();
                // new ArrayList<>();
                ArrayList<PuzzleBoard> solvedList = tempBoard.getPreviousBoard();
                Collections.reverse(solvedList);
                animation = solvedList;
                invalidate();
                return;
            }
            ArrayList<PuzzleBoard> boardList = tempBoard.neighbours();
